...
func Reservasi (r *gin.Engine, db *gorm.DB) {
	// r.GET("/reservasi", session.VerifyToken(), func(c *gin.Context) {
	// 	c.HTML(http.StatusOK, "reservasi.html", nil)
	// })

	r.POST("/reservasi", session.VerifyToken(), func(c *gin.Context) {
	...
	r.GET("/reservasi/api/byID/:id", session.VerifyToken(), func(c *gin.Context) {
		id := c.Param("id")
	...
	r.PUT("/reservasi/api/byID/:id", session.VerifyToken(), func(c *gin.Context) {
		id := c.Param("id")
	...
	r.DELETE("/reservasi/api/byID/:id", session.VerifyToken(), func(c *gin.Context) {
		id := c.Param("id")
